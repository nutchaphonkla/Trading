/* ONE MONTH V12.1 STABLE — standalone runtime */
'use strict';
window.__ONE_MONTH_VERSION__='12.1-stable';

/* ===== indicators.js ===== */
const clamp=(v,min,max)=>Math.max(min,Math.min(max,v));
function ema(values,p){if(!values.length)return[];const k=2/(p+1),out=[];let x=values[0];values.forEach((v,i)=>{x=i?v*k+x*(1-k):v;out.push(x)});return out}
function rsi(values,p=14){if(values.length<=p)return 50;let g=0,l=0;for(let i=1;i<=p;i++){const d=values[i]-values[i-1];g+=Math.max(0,d);l+=Math.max(0,-d)}let ag=g/p,al=l/p;for(let i=p+1;i<values.length;i++){const d=values[i]-values[i-1];ag=(ag*(p-1)+Math.max(0,d))/p;al=(al*(p-1)+Math.max(0,-d))/p}return al===0?100:100-100/(1+ag/al)}
function trueRanges(c){const out=[c[0]?.high-c[0]?.low||0];for(let i=1;i<c.length;i++)out.push(Math.max(c[i].high-c[i].low,Math.abs(c[i].high-c[i-1].close),Math.abs(c[i].low-c[i-1].close)));return out}
function wilders(values,p=14){if(values.length<p)return[];const out=new Array(values.length).fill(null);let x=values.slice(0,p).reduce((a,b)=>a+b,0)/p;out[p-1]=x;for(let i=p;i<values.length;i++){x=(x*(p-1)+values[i])/p;out[i]=x}return out}
function atr(c,p=14){const a=wilders(trueRanges(c),p);return a.at(-1)||0}
...
function macdHist(values){const a=ema(values,12),b=ema(values,26),line=values.map((_,i)=>a[i]-b[i]),sig=ema(line,9);return line.at(-1)-sig.at(-1)}
function pivots(c,left=2,right=2){const highs=[],lows=[];for(let i=left;i<c.length-right;i++){let hi=true,lo=true;for(let j=i-left;j<=i+right;j++)if(j!==i){if(c[j].high>=c[i].high)hi=false;if(c[j].low<=c[i].low)lo=false}if(hi)highs.push({i,price:c[i].high,ts:c[i].ts});if(lo)lows.push({i,price:c[i].low,ts:c[i].ts})}return{highs,lows}}
function structure(c){const p=pivots(c),hs=p.highs.slice(-3),ls=p.lows.slice(-3),last=c.at(-1);let state='RANGE',bos='NONE';if(hs.length>=2&&ls.length>=2){const hh=hs.at(-1).price>hs.at(-2).price,hl=ls.at(-1).price>ls.at(-2).price,lh=hs.at(-1).price<hs.at(-2).price,ll=ls.at(-1).price<ls.at(-2).price;if(hh&&hl)state='BULL';if(lh&&ll)state='BEAR'}const h=hs.at(-1)?.price??null,l=ls.at(-1)?.price??null;if(h&&last.close>h)bos='BULL';if(l&&last.close<l)bos='BEAR';return{state,bos,swingHigh:h,swingLow:l,pivots:p}}

/* ===== storage.js ===== */
const PLAN_KEY='one_month_trading_os_v4';
const SETTINGS_KEY='one_month_studio_settings_v11';
const AI_KEY='one_month_ai_enabled_v11';
const API_USAGE_KEY='one_month_api_usage_v11';
const CACHE_KEY='one_month_market_cache_v11';
const ACTIVE_SIGNAL_KEY='one_month_active_signal_v11';
...
function savePlan(plan){writeJSON(PLAN_KEY,plan)}
function clearPlan(){localStorage.removeItem(PLAN_KEY)}
function loadSettings(){return {...{apiKey:'',teKey:'',riskPct:.5,minLot:.01,contractSize:100,lotFactor:.7},...readJSON(SETTINGS_KEY,{})}}
function saveSettings(s){writeJSON(SETTINGS_KEY,s)}
function aiEnabled(){return localStorage.getItem(AI_KEY)==='1'}
function setAiEnabledStorage(v){localStorage.setItem(AI_KEY,v?'1':'0')}
...
function addApiUsage(n){const x=apiUsage();x.count+=n;x.lastScan=n;writeJSON(API_USAGE_KEY,x);return x}
function resetLastScan(){const x=apiUsage();x.lastScan=0;writeJSON(API_USAGE_KEY,x)}

function loadMarketCache(){return readJSON(CACHE_KEY,{})}
function saveMarketCache(cache){writeJSON(CACHE_KEY,cache)}
...
function loadActiveSignal(){return readJSON(ACTIVE_SIGNAL_KEY,null)}
function saveActiveSignal(s){if(s)writeJSON(ACTIVE_SIGNAL_KEY,s);else localStorage.removeItem(ACTIVE_SIGNAL_KEY)}

/* ===== market.js ===== */
const TF={M1:{api:'1min',ttl:55_000},M5:{api:'5min',ttl:270_000},M15:{api:'15min',ttl:840_000},H1:{api:'1h',ttl:3_300_000}};
...
function filterGoldNews(events=[]){
...
function nextNews(events=[],now=new Date()){
...
function newsLock(events=[],now=new Date()){
...
function chooseSetup(results,{news={lock:false},memory=null,marketOpen=true}={}){
...
function memoryStats(signals,setup){
...
function riskGuardian(plan,signals){
...
function lotForSetup(setup,{balance,maxLoss,contractSize=100,minLot=.01,lotFactor=.7,guardian={mult:1},riskPct=.5}={}){
...
function fingerprint(signal){return[signal.setupType,signal.tf,signal.session,signal.regime,signal.direction||signal.side].join('|')}
...
async function init(){
  try{
    ...
    console.warn('Signal DB unavailable, continuing without memory',e);
    signals=[];
  }
  const validPlan=!!(plan&&Number(plan.start)>0&&Number(plan.target)>Number(plan.start)&&Array.isArray(plan.days)&&plan.days.length===30);
  if(!validPlan){plan=null;clearPlan()}
  renderMarketMode();renderApi();
  await Promise.allSettled([loadStatic(),loadNews()]);
  if(!plan)openModal('#onboardingModal');else{recalcPlan();renderAll()}
  setAiSwitch();renderAi();processLatestCandle();
}
...
setupOnboarding();
init().catch(err=>{
  console.error('ONE MONTH init failed',err);
  const toastEl=document.querySelector('#toast');
  if(toastEl){toastEl.textContent='โหลดระบบไม่สำเร็จ · รีเฟรชหน้าอีกครั้ง';toastEl.classList.add('show')}
});
