...
const pack=JSON.parse(fs.readFileSync(INPUT,'utf8')),raw=pack.timeframes||pack.data||pack||{},m1=clean(raw.M1||[]),m15=m1.length>=300?aggregate(m1,900000):clean(raw.M15||[]);
if(m15.length<260){console.error('Need at least 260 M15 candles');process.exit(1)}
const close=m15.map(x=>x.close),E20=ema(close,20),E50=ema(close,50),E200=ema(close,200),R=rsi(close),A=wild(tr(m15),14),D=adx(m15),atrMedian=[];
const groups=new Map(),samples=[],horizons={M15:{n:0,w:0,r:0},M30:{n:0,w:0,r:0},M60:{n:0,w:0,r:0}};
for(let i=220;i<m15.length-5;i++){
  const atr=A[i]||tr(m15)[i]||1;if(!atr)continue;
  const local=A.slice(Math.max(0,i-80),i+1).filter(Number.isFinite),med=median(local)||atr;atrMedian[i]=med;
  const S=structure(m15,i),adxv=D[i]||0;
  ...
  const direction=bull>=bear?'BUY':'SELL',sess=session(m15[i].ts),rb=bucketRsi(R[i]??50),key=[direction,regime,sess,rb,S].join('|');
  const rawProb=clamp(52+Math.abs(bull-bear)*4+(adxv>=25?5:0)+(S===(direction==='BUY'?'BULL':'BEAR')?4:0)-(regime==='RANGE'?6:0),52,78)/100;
  const o1=firstTouch(m15,i,1,direction,atr),o2=firstTouch(m15,i,2,direction,atr),o4=firstTouch(m15,i,4,direction,atr);
...
const i=m15.length-1,atr=A[i]||1,S=structure(m15,i),adxv=D[i]||0;let regime=adxv>=27?'TREND':adxv<17?'RANGE':'TRANSITION';if(atr>(median(A.slice(-80).filter(Number.isFinite))||atr)*1.75)regime='VOLATILE';let bull=0,bear=0;close[i]>E20[i]?bull++:bear++;E20[i]>E50[i]?bull++:bear++;E50[i]>E200[i]?bull++:bear++;if(S==='BULL')bull++;if(S==='BEAR')bear++;const direction=bull>=bear?'BUY':'SELL',sess=session(m15[i].ts),key=[direction,regime,sess,bucketRsi(R[i]??50),S].join('|'),match=edges.find(e=>e.key===key)||edges.filter(e=>e.direction===direction&&e.regime===regime).sort((a,b)=>b.samples-a.samples)[0]||null;
const out={version:'1.0',engine:'ONEMONTH-SELF-LEARNING-V1',generatedAt:new Date().toISOString(),sourceGeneratedAt:pack.generatedAt||null,ready:true,global:{samples:total,hitRate:Number(hit.toFixed(2)),avgR:Number((samples.reduce((s,x)=>s+x.r,0)/total).toFixed(4)),brier:Number(brier.toFixed(4)),calibrationError:Number(cal.toFixed(2)),reliability:Number(clamp(Math.log10(total+1)/4,0,1).toFixed(4))},horizons:Object.fromEntries(Object.entries(horizons).map(([k,r])=>[k,{samples:r.n,hitRate:Number((r.w/r.n*100).toFixed(2)),avgR:Number((r.r/r.n).toFixed(4))}])),models:{bestRegime:bestBy('regime'),bestSession:bestBy('session'),weights},current:{key,direction,regime,session:sess,structure:S,rsi:Number((R[i]??50).toFixed(2)),learnedWinProbability:Number((match?.posteriorWinRate??50).toFixed(2)),samples:match?.samples||0,reliability:Number((match?.reliability||0).toFixed(4))},edges:edges.filter(e=>e.samples>=4).slice(0,600)};
