...
for(let i=220;i<candles.length-Math.max(...Object.values(horizonBars))-1;i++){
  ...
  samples.push({ts:candles[i].ts,win:o2.win,prob:rawProb,r:o2.r,regime,sess,S,rb,direction});
}
...
const validationStart=Math.max(0,Math.floor(samples.length*.75)),validationRows=samples.slice(validationStart);
const validationWins=validationRows.filter(x=>x.win).length,validationHit=validationRows.length?validationWins/validationRows.length*100:50;
const validationBrier=validationRows.length?validationRows.reduce((s,x)=>s+(x.prob-(x.win?1:0))**2,0)/validationRows.length:0;
let validationCal=0;if(validationRows.length){const vb=new Map();for(const x of validationRows){const b=Math.round(x.prob*10)/10,r=vb.get(b)||{n:0,w:0,p:0};r.n++;r.w+=x.win?1:0;r.p+=x.prob;vb.set(b,r)}for(const r of vb.values())validationCal+=r.n/validationRows.length*Math.abs(r.w/r.n-r.p/r.n);validationCal*=100}
