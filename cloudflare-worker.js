...
async function cleanupOld(env, newestTs) {
  const cutoff = newestTs - RETENTION_DAYS * 24 * 60 * 60 * 1000;
...
async function handleBatch(request, env, ctx) {
  ...
  const receivedAt = Date.now();
  await env.DB.batch(accepted.map(b => insertStatement(env, b, receivedAt)));
  const newestTs = Math.max(...accepted.map(x => x.ts));
  // Cleanup is deliberately based on the newest real candle currently present, not an old backfill page.
  ctx.waitUntil((async () => {
    const newest = await env.DB.prepare('SELECT MAX(ts) AS ts FROM bars').first();
    if (Number(newest?.ts) > 0) await cleanupOld(env, Number(newest.ts));
  })().catch(console.error));
  return json({
...
function safeLimit(v, max = 10_000, fallback = 6000) {
  const n = Number(v);
  return Math.min(max, Math.max(100, Number.isFinite(n) ? Math.floor(n) : fallback));
}
...
async function buildFeed(request, env) {
  await assertDatabaseReady(env);
  const url = new URL(request.url), limit = safeLimit(url.searchParams.get('limit') || 6000, 10_000, 6000);
  const result = await env.DB.prepare(`
    SELECT ts, symbol, timeframe, open, high, low, close, received_at
    FROM bars ORDER BY ts DESC LIMIT ?
  `).bind(limit).all();
  const rows = mapRows((result.results || []).reverse());
  const last = rows.at(-1) || null;
  return json({
    status: 'ok', version: VERSION, provider: 'MT5 auto-hybrid bridge', source: 'MT5 -> Cloudflare Worker -> D1',
...
async function buildHistory(request, env) {
  if (!authorized(request, env)) return json({ status: 'error', message: 'unauthorized' }, 401);
  await assertDatabaseReady(env);
  const url = new URL(request.url);
  ...
  const limit = safeLimit(url.searchParams.get('limit') || 5000, 5_000, 5000);
  const result = await env.DB.prepare(`
    SELECT ts, symbol, timeframe, open, high, low, close, received_at
    FROM bars WHERE ts < ? ORDER BY ts DESC LIMIT ?
  `).bind(before, limit).all();
  const rows = mapRows((result.results || []).reverse());
  return json({
    status: 'ok', version: VERSION, provider: 'MT5 historical academy',
    generatedAt: new Date().toISOString(), count: rows.length,
    before, oldestTs: rows[0]?.ts ?? null, newestTs: rows.at(-1)?.ts ?? null,
...
async function handleHealth(request, env) {
  ...
  const oldest = await env.DB.prepare('SELECT MIN(ts) AS ts FROM bars').first();
  if (!latest) return json({ status: 'empty', version: VERSION, provider: 'MT5 auto-hybrid bridge', database: 'connected', table: 'bars', bars: 0, latest: null, retentionDays: RETENTION_DAYS, message: 'Worker + D1 ready. Waiting for MT5 XAUUSD M1 bars.' });
  const ts = Number(latest.ts), receivedAt = Number(latest.received_at);
  return json({
    status: 'ok', version: VERSION, provider: 'MT5 auto-hybrid bridge', database: 'connected', table: 'bars', bars: Number(count?.n || 0),
    oldestTs: Number(oldest?.ts || 0) || null, retentionDays: RETENTION_DAYS,
...
export default {
  async fetch(request, env, ctx) {
    ...
    const url = new URL(request.url);
    try {
      if (request.method === 'GET' && url.pathname === '/') return json({ status: 'ok', version: VERSION, service: 'OneMonth MT5 auto-hybrid academy bridge', database: 'Cloudflare D1', retentionDays: RETENTION_DAYS, endpoints: ['/mt5-webhook', '/tv-webhook', '/mt5-batch', '/feed', '/public-feed', '/history', '/health'] });
      ...
      if (request.method === 'GET' && url.pathname === '/history') return await buildHistory(request, env);
