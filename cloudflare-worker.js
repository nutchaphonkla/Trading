...
function validateBar(body) {
  ...
  if (high < Math.max(open, close) || low > Math.min(open, close) || high < low) {
    return { ok: false, error: 'malformed OHLC' };
  }
  ...
  if (!new Set(['1', '1M', '1MIN', '1MINUTE']).has(timeframe)) {
    return { ok: false, error: 'timeframe must be 1 minute' };
  }
...
async function handleWebhook(request, env, ctx) {
  ...
  catch { return json({ status: 'error', message: 'body must be valid JSON' }, 400); }
  ...
  `).bind(bar.ts, bar.symbol, bar.open, bar.high, bar.low, bar.close, receivedAt).run();

  if (bar.ts % 3_600_000 < 60_000) {
    const cutoff = bar.ts - 14 * 24 * 60 * 60 * 1000;
    ctx.waitUntil(env.DB.prepare('DELETE FROM bars WHERE ts < ?').bind(cutoff).run().catch(console.error));
  }
  ...
  return json({
    ...
    accepted: true,
    provider: 'MT5',
    source: 'MT5 -> Cloudflare Worker -> D1',
    receivedAt,
    bar: {
      ts: bar.ts,
      datetime: new Date(bar.ts).toISOString(),
      symbol: bar.symbol,
      timeframe: 'M1',
      open: bar.open,
      high: bar.high,
...
async function buildFeed(request, env) {
  await assertDatabaseReady(env);
  const url = new URL(request.url);
  const limit = safeLimit(url.searchParams.get('limit') || 6000);
  ...
  return json({
    status: 'ok',
    provider: 'MT5 fallback bridge',
    source: 'MT5 -> Cloudflare Worker -> D1',
    generatedAt: new Date().toISOString(),
    symbol: last ? 'XAU/USD' : null,
    count: rows.length,
    latestTs: last?.ts ?? null,
    latestAgeMs: last ? Math.max(0, Date.now() - last.ts) : null,
    timeframes: { M1: rows },
  });
}
...
async function handleHealth(request, env) {
  ...
  const latest = await env.DB.prepare(`
    SELECT ts, symbol, open, high, low, close, received_at
    FROM bars ORDER BY ts DESC LIMIT 1
  `).first();
  const count = await env.DB.prepare('SELECT COUNT(*) AS n FROM bars').first();
  if (!latest) {
    return json({
      status: 'empty', version: 'V36.2', provider: 'MT5 fallback bridge',
      database: 'connected', table: 'bars', bars: 0, latest: null,
      message: 'Worker + D1 ready. Waiting for MT5 XAUUSD M1 bars.'
    });
  }
  const ts = Number(latest.ts), receivedAt = Number(latest.received_at);
  return json({
    status: 'ok', version: 'V36.2', provider: 'MT5 fallback bridge',
    database: 'connected', table: 'bars', bars: Number(count?.n || 0),
    latest: {
      ts,
      ...
      receivedAt,
      candleAgeMs: Math.max(0, Date.now() - ts),
      receiveAgeMs: Math.max(0, Date.now() - receivedAt),
    },
  });
}
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (request.method === 'GET' && url.pathname === '/') {
        return json({
          status: 'ok', version: 'V36.2 PRIMARY FAILOVER',
          service: 'OneMonth MT5 fallback bridge', database: 'Cloudflare D1',
          endpoints: ['/tv-webhook', '/feed', '/public-feed', '/health']
        });
      }
      if (request.method === 'POST' && url.pathname === '/tv-webhook') return await handleWebhook(request, env, ctx);
